...
fn pcap_filter_to_sim_packet(parsed: &pcap_analyze::ParsedPacket, raw: &[u8]) -> simulator::SimPacket {
    ...
    simulator::SimPacket {
        ...
        vlan_pcp: parsed.vlan_pcp,
        outer_vlan_id: parsed.outer_vlan_id,
        outer_vlan_pcp: parsed.outer_vlan_pcp,
        src_ip: parsed.src_ip.map(|ip| ip.to_string()),
        dst_ip: parsed.dst_ip.map(|ip| ip.to_string()),
        ip_protocol: parsed.ip_protocol,
        ip_dscp: parsed.ip_dscp,
        ip_ecn: parsed.ip_ecn,
        ip_ttl: parsed.ip_ttl,
        ip_dont_fragment: parsed.ip_dont_fragment,
        ip_more_fragments: parsed.ip_more_fragments,
        ip_frag_offset: parsed.ip_frag_offset,
        src_ipv6: parsed.src_ipv6.map(|ip| ip.to_string()),
        dst_ipv6: parsed.dst_ipv6.map(|ip| ip.to_string()),
        ipv6_next_header: parsed.ipv6_next_header,
        ipv6_dscp: parsed.ipv6_dscp,
        ipv6_ecn: parsed.ipv6_ecn,
        ipv6_hop_limit: parsed.ipv6_hop_limit,
        ipv6_flow_label: parsed.ipv6_flow_label,
        src_port: parsed.src_port,
        dst_port: parsed.dst_port,
        tcp_flags: parsed.tcp_flags,
        icmp_type: parsed.icmp_type,
        icmp_code: parsed.icmp_code,
        icmpv6_type: parsed.icmpv6_type,
        icmpv6_code: parsed.icmpv6_code,
        igmp_type: parsed.igmp_type,
        mld_type: parsed.mld_type,
        arp_opcode: parsed.arp_opcode,
        arp_spa: parsed.arp_spa.map(|ip| ip.to_string()),
        arp_tpa: parsed.arp_tpa.map(|ip| ip.to_string()),
        vxlan_vni: parsed.vxlan_vni,
        gtp_teid: parsed.gtp_teid,
        geneve_vni: parsed.geneve_vni,
        gre_protocol: parsed.gre_protocol,
        gre_key: parsed.gre_key,
        mpls_label: parsed.mpls_label,
        mpls_tc: parsed.mpls_tc,
        mpls_bos: parsed.mpls_bos,
        oam_level: parsed.oam_level,
        oam_opcode: parsed.oam_opcode,
        nsh_spi: parsed.nsh_spi,
        nsh_si: parsed.nsh_si,
        nsh_next_protocol: parsed.nsh_next_protocol,
        ptp_message_type: parsed.ptp_message_type,
        ptp_domain: parsed.ptp_domain,
        ptp_version: parsed.ptp_version,
