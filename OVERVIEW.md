...
## What It Does
1. You define packet filter rules in YAML (match on MAC, IPv4/IPv6, ports, VLAN, QinQ double VLAN, VXLAN VNI, GTP-U TEID, MPLS labels, IGMP/MLD, DSCP/ECN, IPv6 TC, TCP flags, ICMP type/code, ICMPv6 type/code, ARP opcode/SPA/TPA, IPv6 hop_limit/flow_label, IPv4 fragmentation flags, etc.) with optional rewrite actions (NAT, TTL, MAC, VLAN, DSCP, L4 port rewrite)
...
## Architecture
The generated hardware has a simple streaming interface (byte-at-a-time Ethernet frames). A hand-written frame parser extracts L2/L3/L4/VXLAN/GTP-U/MPLS/IGMP/MLD/ICMP/ICMPv6/ARP header fields (including QinQ outer VLAN, IPv6 Traffic Class, TCP flags, hop_limit, flow_label, IPv4 fragmentation flags), generated per-rule matchers evaluate in parallel (combinational), and a priority encoder selects the first matching rule's action (pass or drop).
...
- `packet_filter_axi_top` — AXI-Stream top-level (hand-written)
  ...
  - `packet_filter_top` — generated top-level, wires everything
    - `frame_parser` — hand-written, extracts L2/L3/L4/QinQ/VXLAN/GTP-U/MPLS/IGMP/MLD/ICMP/ICMPv6/ARP fields + IPv6 TC/hop_limit/flow_label + TCP flags + IPv4 frag
