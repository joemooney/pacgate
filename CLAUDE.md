...
- **Native wide parser**: `--width >= 512` with `--axi` activates combinational parallel field extraction (1-2 cycles vs 54+ for serial parser); foundation for future 100G+ throughput
...
- `lint` subcommand for best-practice analysis and security checks (58 lint rules)
...
- `packet_filter_top` — generated top-level
  ...
  - `frame_parser_wide` — generated combinational wide-bus parser: parallel L2/L3/L4/tunnel/protocol extraction from 512+ bit AXI-Stream (templates/frame_parser_wide.v.tera)
...
pacgate compile rules.yaml --axi --width 512   # Native wide parser (1-2 cycle extraction)
...
- `templates/frame_parser_wide.v.tera` — Combinational wide-bus parser template (512+ bit parallel extraction, 3-way VLAN mux, word-1 pipeline)
...
- Wide parser (--width >= 512): combinational parallel field extraction from wide AXI-Stream bus in 1-2 clock cycles; 3-way mux for untagged/VLAN/QinQ offsets; word-1 pipeline for IPv6+TCP flags that overflow 64-byte word 0; same output port list as frame_parser.v for drop-in replacement; FIFO and rewrite engine remain 8-bit (end-to-end throughput still ~2 Gbps until those are widened)
...
- **Phase 38**: Complete — Native Wide Parser (`--width >= 512`):
  - **38.1 Generator Infrastructure**: `generate()` and `copy_axi_rtl()` accept `wide_parser`/`data_width` params, `generate_wide_parser()` renders template
  - **38.2+38.3 Wide Parser Template**: `frame_parser_wide.v.tera` (~520 LOC) — combinational parallel extraction with word-0 (all IPv4/ARP/OAM/NSH/MPLS/tunnels) and word-1 pipeline (IPv6+TCP flags)
  - **38.4 Template Integration**: `packet_filter_top.v.tera` and `packet_filter_axi_top.v.tera` conditional wide parser instantiation with `wide_sof` generation
...
- **Phase 40**: Complete — Packet Match Trace (`trace` subcommand):
  - **40.1 Core Module**: `src/trace.rs` (~400 LOC) — evaluates ALL rules against a packet with per-rule, per-field pass/fail breakdown; `TraceResult`/`RuleTrace`/`StageTrace`/`PipelineTrace` structs; text and JSON formatters; pipeline stage-by-stage trace
